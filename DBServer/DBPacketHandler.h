...
public:
	static void Init()
	{
		...
		GPacketHandler[PKT_SD_GET_INFOMATION] = [](PacketSessionRef& session, BYTE* buffer, int32 len) { return HandlePacket<DBProtocol::SD_GET_INFOMATION>(Handle_SD_GET_INFOMATION, session, buffer, len); };
		GPacketHandler[PKT_SD_EQUIP_ITEM] = [](PacketSessionRef& session, BYTE* buffer, int32 len) { return HandlePacket<DBProtocol::SD_EQUIP_ITEM>(Handle_SD_EQUIP_ITEM, session, buffer, len); };
		GPacketHandler[PKT_SD_FARMING_ITEM] = [](PacketSessionRef& session, BYTE* buffer, int32 len) { return HandlePacket<DBProtocol::SD_FARMING_ITEM>(Handle_SD_FARMING_ITEM, session, buffer, len); };
		GPacketHandler[PKT_SD_UNEQUIP_ITEM] = [](PacketSessionRef& session, BYTE* buffer, int32 len) { return HandlePacket<DBProtocol::SD_UNEQUIP_ITEM>(Handle_SD_UNEQUIP_ITEM, session, buffer, len); };
		GPacketHandler[PKT_SD_CONSUME_ITEM] = [](PacketSessionRef& session, BYTE* buffer, int32 len) { return HandlePacket<DBProtocol::SD_CONSUME_ITEM>(Handle_SD_CONSUME_ITEM, session, buffer, len); };
		GPacketHandler[PKT_SD_MOVE_ITEM] = [](PacketSessionRef& session, BYTE* buffer, int32 len) { return HandlePacket<DBProtocol::SD_MOVE_ITEM>(Handle_SD_MOVE_ITEM, session, buffer, len); };
		GPacketHandler[PKT_SD_REGISTER] = [](PacketSessionRef& session, BYTE* buffer, int32 len) { return HandlePacket<DBProtocol::SD_REGISTER>(Handle_SD_REGISTER, session, buffer, len); };
		GPacketHandler[PKT_SD_SAVE_INVENTORY] = [](PacketSessionRef& session, BYTE* buffer, int32 len) { return HandlePacket<DBProtocol::SD_SAVE_INVENTORY>(Handle_SD_SAVE_INVENTORY, session, buffer, len); };
		GPacketHandler[PKT_SD_SAVE_EQUIPMENT] = [](PacketSessionRef& session, BYTE* buffer, int32 len) { return HandlePacket<DBProtocol::SD_SAVE_EQUIPMENT>(Handle_SD_SAVE_EQUIPMENT, session, buffer, len); };
		GPacketHandler[PKT_SD_UPDATE_GOLD] = [](PacketSessionRef& session, BYTE* buffer, int32 len) { return HandlePacket<DBProtocol::SD_UPDATE_GOLD>(Handle_SD_UPDATE_GOLD, session, buffer, len); };
	}
